...
export default function WeekOffPicker({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-[13px] font-medium text-slate-700">Off day(s)</label>
      <div className="flex gap-2 mb-2">
        <button
          type="button"
          onClick={() => setType("fixed")}
          className={`flex-1 rounded-md border px-3 py-2 text-[13px] font-medium transition-colors ${
            value.week_off_type === "fixed" ? "border-slate-800 bg-slate-100 text-slate-900" : "border-slate-300 text-slate-600"
          }`}
        >
          Fixed days off
        </button>
        <button
          ...
          className={`flex-1 rounded-md border px-3 py-2 text-[13px] font-medium transition-colors ${
            value.week_off_type === "rotational" ? "border-slate-800 bg-slate-100 text-slate-900" : "border-slate-300 text-slate-600"
          }`}
        >
          Rotational off
        </button>
      ...
      {value.week_off_type === "fixed" && (
        <div className="grid grid-cols-4 gap-1.5 rounded-md border border-slate-200 p-3">
          {WEEK_DAYS.map((day) => (
          ...
          <div>
            ...
            <div className="flex gap-2">
              {([1, 2] as const).map((n) => (
                <label
                  key={n}
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-[13px] cursor-pointer ${
                    value.rotational_offs_per_week === n ? "border-slate-800 bg-slate-100 text-slate-900" : "border-slate-300 text-slate-600"
                  }`}
                >
                  <input
                    type="radio"
